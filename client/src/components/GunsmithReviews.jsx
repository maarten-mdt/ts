...
export default function GunsmithReviews({ gunsmith }) {
  ...
  const { data: reviewsData } = useQuery({
    queryKey: ['reviews', 'gunsmith', gunsmith.id],
    queryFn: () => api.get('/reviews', { gunsmithId: gunsmith.id }),
  })
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['reviews', 'gunsmith', gunsmith.id] })
      queryClient.invalidateQueries({ queryKey: ['gunsmith', gunsmith.slug] })
      setShowForm(false)
      setBody('')
      setVisitDate('')
    },
    ...
    <section>
        ...
        <form onSubmit={handleSubmit} className="mb-6 p-4 rounded-lg border border-stone-700 bg-surface-elevated space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              placeholder="Share your experience with this gunsmith..."
      ...
      <div className="space-y-4">
        {reviews.length === 0 && !showForm && (
          <p className="text-stone-500">No reviews yet. Be the first to review this gunsmith.</p>
        )}
        {reviews.map((review) => (
          <div
            key={review.id}
